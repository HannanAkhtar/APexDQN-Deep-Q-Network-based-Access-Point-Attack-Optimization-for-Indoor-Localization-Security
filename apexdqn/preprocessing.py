...
def drop_constant_and_weak(X_train):
    """Performs feature selection by removing undetected or weak-signal APs."""
    # Remove APs that are never detected (all RSSI values are 100).
    ...
    # Remove APs with consistently weak signals (max RSSI < -2).
    cols_not_all_100 = X_train.columns[~(X_train == 100).all()]
    cols_to_drop = []
    for c in cols_not_all_100:
        max_non_100 = X_train[c][X_train[c] != 100].max()
        if max_non_100 < -2:
...
def get_ap_columns(X):
    ...
    return X.columns[:-NUM_NON_AP_FEATURES]
