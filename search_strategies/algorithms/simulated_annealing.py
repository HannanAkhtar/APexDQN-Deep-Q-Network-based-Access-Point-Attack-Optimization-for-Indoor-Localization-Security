...
def get_neighbor(current_aps: Sequence[str], all_aps: Sequence[str], k: int, rng: np.random.Generator | None = None) -> list[str]:
    """Generate a neighbor by swapping one selected AP with one unselected AP."""
...
def run_simulated_annealing(
    ...
    *,
    ...
    """Run the simulated annealing benchmark and return one summary row per k."""
    ...
    for k in range(1, len(ap_columns) + 1):
        ...
        for _ in range(num_trials):
            current_aps = list(rng.choice(ap_columns, size=k, replace=False))
            current_mse = evaluate_attack_mse(model, X_val, y_val, current_aps, noise, scaler=scaler, predict_kwargs=predict_kwargs)
            ...
            while temperature > T_min:
                for _ in range(max_iter_per_temp):
                    neighbor_aps = get_neighbor(current_aps, ap_columns, k, rng=rng)
                    neighbor_mse = evaluate_attack_mse(model, X_val, y_val, neighbor_aps, noise, scaler=scaler, predict_kwargs=predict_kwargs)
                    delta = neighbor_mse - current_mse
